...
<div class="max-w-7xl mx-auto space-y-6 animate-fade-up">
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div class="px-8 py-6 border-b border-gray-100 bg-gray-50/50 flex items-center gap-4">
            ...
            <div>
                <h2 class="font-heading text-2xl font-bold text-slate-800 uppercase">Actualizar Tipo de Unidad: {{ $tipo_unidad->nombre }}</h2>
            </div>
        </div>

        {{-- Cuerpo del Formulario --}}
        <form action="{{ route('tipo_unidades.update', $tipo_unidad->id) }}" method="POST" class="p-8 space-y-6">
            @csrf
            @method('PUT')
            <div class="space-y-2">
                <label for="nombre" class="block text-xs font-bold text-slate-500 uppercase tracking-widest">
                    Nombre o Identificador de Tipo de Unidad
                </label>
                <div class="relative group">
                    ...
                    <input type="text" 
                           name="nombre" 
                           id="nombre" 
                           value="{{ old('nombre', $tipo_unidad->nombre) }}" {{-- Carga el valor actual o lo que el usuario escribió si hubo error --}}
    ...
    <div class="bg-amber-50/50 border border-amber-100 rounded-xl p-4 flex items-center gap-3">
        ...
        <p class="text-md text-amber-700 leading-relaxed text-center">
            <strong>Atención:</strong> Al cambiar el nombre del tipo de unidad, todos los registros históricos asociados a este ID se actualizarán.
        </p>
    </div>
