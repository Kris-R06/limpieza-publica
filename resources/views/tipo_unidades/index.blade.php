...
<div class="space-y-6 animate-fade-up">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="font-heading text-3xl sm:text-4xl font-extrabold text-brand-850 tracking-wide">Lista de Tipos de Unidades</h1>
            <p class="text-sm text-slate-500">Administración de los tipos de unidades del municipio.</p>
        </div>
        
        <a href="{{ route('tipo_unidades.create') }}" 
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($tipo_unidades as $tipo_unidad)
                    <tr class="hover:bg-gray-50/50 transition-colors group">
                        <td class="px-6 py-4">
                            <span class="text-sm font-medium text-slate-400">#{{ str_pad($tipo_unidad->id, 3, '0', STR_PAD_LEFT) }}</span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                <i class="ph ph-truck text-brand-600"></i>
                                <span class="text-sm font-bold text-brand-700 tracking-wide uppercase">{{ $tipo_unidad->nombre }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-right">
                            <div class="inline-flex items-center gap-2">
                                {{-- Botón Editar --}}
                                <a href="{{ route('tipo_unidades.edit', $tipo_unidad->id) }}" 
                                ...
                                {{-- Botón Eliminar --}}
                                <form action="{{ route('tipo_unidades.destroy', $tipo_unidad->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" 
                                            onclick="return confirm('¿Estás seguro de eliminar este tipo de unidad?')"
                    ...
                    <tr>
                        <td colspan="3" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center gap-3">
                                ...
                                <p class="text-slate-400 text-sm">No se encontraron tipos de unidades registrados.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        @if(method_exists($tipo_unidades, 'links'))
