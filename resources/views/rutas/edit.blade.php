...
<body class="bg-gray-50 text-gray-800 font-sans antialiased flex flex-col min-h-screen">
    ...
    <main class="flex-grow">
        <div class="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
            <h1 class="text-3xl font-bold text-gray-900 mb-6">Editar Ruta {{ $ruta->numero }}</h1>
            <form action="{{ route('rutas.update', $ruta->id) }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
                @csrf
                @method('PUT')
                <div class="mb-4">
                    <label for="numero" class="block text-gray-700 text-sm font-bold mb-2">Número de Ruta</label>
                    <input type="text" name="numero" id="numero" value="{{ $ruta->numero }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                </div>
                <div class="flex items-center justify-between">
                    <button type="submit" class="bg-eco hover:bg-green-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">
                        Actualizar Ruta
                    </button>
                    <a href="{{ route('rutas.index') }}" class="inline-block align-baseline font-bold text-sm text-green-500 hover:text-green-800">
                        Cancelar
                    </a>
                </div>
            </form>            
