...
<div class="max-w-7xl mx-auto space-y-6 animate-fade-up">
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div class="px-8 py-6 border-b border-gray-100 bg-gray-50/50 flex items-center gap-4">
            ...
            <div>
                <h2 class="font-heading text-2xl font-bold text-slate-800 uppercase">Actualizar Trabajador: {{ $trabajador->nombre }}</h2>
            </div>
        </div>

        {{-- Cuerpo del Formulario --}}
        <form action="{{ route('trabajadores.update', $trabajador->id) }}" method="POST" class="p-8 space-y-6">
            @csrf
            @method('PUT')
            <div class="space-y-2">
                ...
                <div class="relative group">
                    ...
                    <input type="text" 
                           name="nombre" 
                           id="nombre" 
                           value="{{ old('nombre', $trabajador->nombre) }}" {{-- Carga el valor actual o lo que el usuario escribió si hubo error --}}
                ...
                <div class="relative group">
                    <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400 group-focus-within:text-brand-500 transition-colors">
                        <i class="ph ph-hard-hat text-lg"></i>
                    </div>
                    <select name="tipo" id="tipo" class="block w-full pl-11 pr-10 py-3 bg-gray-50 border border-gray-200 rounded-xl text-slate-700 font-medium focus:ring-4 focus:ring-brand-500/10 focus:border-brand-500 transition-all outline-none" required>
                        ...
                        @foreach($tipos as $tipo)
                            <option value="{{ $tipo->id }}" {{ old('tipo', $trabajador->tipo_trabajador_id) == $tipo->id ? 'selected' : '' }}>{{ $tipo->nombre }}
                            </option>
                        @endforeach
                    </select>
                </div>
