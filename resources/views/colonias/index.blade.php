...
<div class="space-y-6 animate-fade-up">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="font-heading text-3xl sm:text-4xl font-extrabold text-brand-850 tracking-wide">Lista de Colonias</h1>
            <p class="text-sm text-slate-500">Administración de las colonias del municipio.</p>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50/50">
                        ...
                        <th class="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] border-b border-gray-100">Nombre de Colonia</th>
                        <th class="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] border-b border-gray-100">Habitantes</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors group">
                        <td class="px-6 py-4">
                            <span class="text-sm font-medium text-slate-400">#{{ str_pad($colonia->id, 3, '0', STR_PAD_LEFT) }}</span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                <i class="ph ph-path text-brand-600"></i>
                                <span class="text-sm font-bold text-brand-700 tracking-wide uppercase">{{ $colonia->nombre }}</span>
                            </div>
                        </td>
                        <td>
                            <div class="flex items-center gap-2">
                                <i class="ph ph-path text-brand-600"></i>
                                <span class="text-sm font-bold text-brand-700 tracking-wide uppercase">{{ $colonia->habitantes }}</span>
                            </div>
                        </td>
                        <td>
                            <div class="flex items-center gap-2">
                                <i class="ph ph-path text-brand-600"></i>
                                <span class="text-sm font-bold text-brand-700 tracking-wide uppercase">{{ $colonia->ruta->numero }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-right">
                            <div class="inline-flex items-center gap-2">
                                {{-- Botón Editar --}}
                                <a href="{{ route('colonias.edit', $colonia->id) }}" 
                                ...
                                {{-- Botón Eliminar --}}
                                <form action="{{ route('colonias.destroy', $colonia->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" 
                                            onclick="return confirm('¿Estás seguro de eliminar esta colonia?')"
